...
def call(player):
    ...
    chips_to_call = current_bet - sum(player_chips.values()) if player == human_player else current_bet - sum(
        bot_chips.values())
    ...
    if player_chips["blanca(1$)"] < chips_to_call:
        ...
        pot += chips_to_call
        print(f"{human_player} iguala la apuesta de {chips_to_call} fichas.")
        print(f"Fichas en juego: {pot}")
...
def evaluate_hands(cards_player, cards_opponent):
    ...
    def royal_flush(hand):
        return straight_flush(hand) and min([card_values[card.split()[0]] if card.split()[0] in card_values else int(
            card.split()[0]) for card in hand]) == 10

    if royal_flush(cards_player):
        return 9
    elif straight_flush(cards_player):
        return 8
    elif four_of_a_kind(cards_player):
        return 7
    elif full_house(cards_player):
        return 6
    elif flush(cards_player):
        return 5
    elif straight(cards_player):
        return 4
    elif three_of_a_kind(cards_player):
        return 3
    elif two_pairs(cards_player):
        return 2
    elif pair(cards_player):
        return 1
    else:
        return 0

def start_game():
    global player, bot, table, pot, current_bet
    player = []
    bot = []
    table = []
    pot = 0
    current_bet = 0
    print("\nComenzando el juego de POKER HOLD'EM...")
